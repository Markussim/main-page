function search() {
  ...
  xhttp.open("GET", "https://marksism.space/search_api.php?search_box=" + document.getElementById('search_box_form').value, true);
  xhttp.send();
}
...
function search_onload() {
  ...
  xhttp.open("GET", "https://marksism.space/search_api.php?search_box=" + url, true);
  xhttp.send();
}
...
function myFunction(arr) {
  if (arr.length >= 1) {
    ...
    for (i = 0; i < arr.length; i++) {
      out += '<a href="' + arr[i].ActualLink + '">' + arr[i].SiteName + '</a><br>' + arr[i].Description + '<br><br>';
    }
